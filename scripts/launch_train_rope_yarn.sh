...
args=(
  ...
  --seq-len "$SEQ_LEN"
  --seq-len-final "$SEQ_LEN_FINAL"
  --max-seq-len "$SEQ_LEN_FINAL"
  --original-seq-len "$SEQ_LEN"
...
uv run torchrun --nproc_per_node=1 --master_port=29502 src/train.py "${args[@]}" "$@"
